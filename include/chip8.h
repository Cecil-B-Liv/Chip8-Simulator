...
int romLoaderNoMaloc(Chip8* chip8, const char* filename) {  // do not use malloc to load rom
    FILE* rom = fopen(filename, "rb");
    if (!rom) {
        perror("Failed to open ROM");
        return -1;
...
int romLoaderTest(Chip8* chip8,
                  const uint8_t* rom,
                  size_t romSize) {  // load rom from data array (for testing)
    if (!rom) {
...
void debug_dump_memory(uint8_t* memory, int start, int length) {  // dump memory for debugging
